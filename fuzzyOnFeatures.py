...
def getQuestionClasses(auto_f, volunteers, question, seed):
    ...
    _,u,_,_,_,_,_ = skfuzzy.cmeans(np.transpose(valid_feats), c=len(fcol_names), m=2, error=1e-9, maxiter=300,seed=seed)
...
for question in label_metadata.decals_pairs:
    ...
    start = time.time()
    for seed in seeds:
        idxs, support, anscols, valid_preds, valid_vol = getQuestionClasses(auto_features, decals_test, schema.get_question(question), seed)
    ...
    total_report[question]['precision'] /= len(seeds)
    total_report[question]['recall'] /= len(seeds)
    total_report[question]['f1'] /= len(seeds)
    total_time[question]['total'] = end - start
    total_time[question]['avg'] = total_time[question]['total']/len(seeds)
    print('Question: ',question,' Completed ', len(seeds), ' times and avearged.')
    print('--------------------------------------------------------------')

report_df = pd.DataFrame.from_dict(total_report, orient='index')
time_df = pd.DataFrame.from_dict(total_time, orient='index')

report_df.to_csv("/users/ezraf/clusterResults/fuzzy_accuracy.csv")
time_df.to_csv("/users/ezraf/clusterResults/fuzzy_time.csv")